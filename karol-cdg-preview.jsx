...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, LineChart, Line, Legend, AreaChart, Area, Cell, ReferenceLine, ComposedChart } from "recharts";
...
const narrative = [
  { uo: 'VLB', alert: 'GIALLO', testo: 'MOL-I 16,4% in target, ma costi sede (17,6% ricavi) assorbono il margine. MOL-G -24k. Azione: rinegoziare allocazione sede o aumentare occupancy oltre 90%.', outlook: 'Strutturale' },
  { uo: 'CTA', alert: 'ROSSO', testo: 'MOL-G -101k. Personale al 59,4% dei ricavi. Il costo medici (362k) il piu alto del gruppo. Azione: ottimizzare mix medici/OSS.', outlook: 'Persistente' },
  { uo: 'COS', alert: 'VERDE', testo: 'Best performer. MOL-I 23%, MOL-G 8,8%. Mix ricavi diversificato. Ricavo/giornata 242 il piu alto.', outlook: 'Favorevole' },
  ...
  { uo: 'KCP', alert: 'ROSSO', testo: 'MOL-G -61k (-8,7%). Diseconomie di scala: 20 PL, sede 22,6% ricavi. Valutare accorpamento con VLB.', outlook: 'Strutturale' },
  { uo: 'GRUPPO', alert: 'ROSSO', testo: 'MOL-G 5,0% sotto target 8%. Costi sede 16,2%. COS+LAB generano margine, VLB/CTA/KCP lo erodono. CF netto -600k per debito (520k) e CAPEX (280k).', outlook: 'Critico - piano efficientamento necessario' },
];
...
const Badge = ({ reale }) => {
  if (reale) return <span style={{ fontSize: 9, padding: '1px 6px', borderRadius: 10, background: C.verdeBg, color: C.verde, marginLeft: 6, fontWeight: 600 }}>Consuntivo</span>;
...
const Semaforo = ({ valore, verdeMin, gialloMin, invertito }) => {
  let colore;
  if (invertito) {
  ...
  const sfondo = colore === C.verde ? C.verdeBg : colore === C.giallo ? C.gialloBg : C.rossoBg;
  return <span style={{ background: sfondo, color: colore, padding: '2px 10px', borderRadius: 12, fontSize: 11, fontWeight: 600 }}>{typeof valore === 'number' ? (valore % 1 === 0 ? valore : valore.toFixed(1)) : valore}</span>;
};

export default function KarolDashboard() {
  ...
  const scost = [...UO].map(u => ({ nome: u.cod, scost: u.molG, fill: u.molG >= 0 ? C.verde : C.rosso })).sort((a,b) => a.scost - b.scost);
    ...
    <div style={{ background: C.sfondo, minHeight: '100vh', fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, ' + C.primario + ' 0%, ' + C.primarioChiaro + ' 100%)', padding: '20px 32px', color: 'white' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 15, fontWeight: 600 }}>Sicilia — Anno 2025 (12 Mesi)</div>
      ...
      <div style={{ maxWidth: 1280, margin: '-20px auto 0', padding: '0 24px', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(155px, 1fr))', gap: 12 }}>
          {[
            ...
            { label: 'Alert', valore: (nR+nG) + ' attivi', sub: nR + ' rossi ' + nG + ' gialli ' + nV + ' verdi', colore: C.rosso },
          ].map((kpi, i) => (
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '20px 24px 40px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div style={cardS}>
                ...
                <p style={{ fontSize: 11, color: C.t3, marginBottom: 16 }}>Divergenza dal breakeven per unita operativa</p>
            ...
            <div style={cardS}>
              ...
              {narrative.filter(n => n.uo === 'GRUPPO').map((n, i) => {
                ...
                return (
                  <div key={i} style={{ padding: '12px 16px', background: ac.bg, borderRadius: 8, borderLeft: '4px solid ' + ac.color }}>
                ...
                <div style={cardS}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                    <div>
                      ...
                      <p style={{ fontSize: 12, color: C.t2 }}>{u.tipo}{u.pl > 0 ? ' — ' + u.pl + ' PL' : ''}</p>
                    </div>
                    <div style={{ textAlign: 'right' }}>
        ...
        {tab === 'cashflow' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={cardS}>
              ...
              <p style={{ fontSize: 11, color: C.t3, marginBottom: 16 }}>Waterfall da Ricavi a Cash Flow Netto</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div style={cardS}>
                <h3 style={{ fontSize: 15, fontWeight: 700, color: C.t1, marginBottom: 4 }}>Saldo Cassa — 12 Settimane</h3>
                <p style={{ fontSize: 11, color: C.t3, marginBottom: 16 }}>Soglia minima 200k (linea tratteggiata)</p>
                <ResponsiveContainer width="100%" height={260}>
                  <AreaChart data={cassaSett} margin={{ top: 10, right: 10, left: 10, bottom: 5 }}>
                    ...
                    <YAxis tickFormatter={v => v + 'k'} fontSize={11} stroke={C.t3} domain={[0, 600]} />
                    <Tooltip formatter={v => [v + 'k']} />
                    <ReferenceLine y={200} stroke={C.rosso} strokeDasharray="6 4" strokeWidth={2} />
                    <Area type="monotone" dataKey="saldo" name="Saldo" stroke={C.CTA} strokeWidth={2.5} fill="url(#gradC)" />
                  </AreaChart>
                </ResponsiveContainer>
                <div style={{ marginTop: 8, padding: '8px 12px', background: C.rossoBg, borderRadius: 6, fontSize: 11, color: C.rosso, fontWeight: 500 }}>
                  S10: Saldo 180k sotto soglia — possibile tensione di cassa per ritardo incassi ASP
                </div>
              </div>

              <div style={cardS}>
                <h3 style={{ fontSize: 15, fontWeight: 700, color: C.t1, marginBottom: 4 }}>Cash Flow Cumulato — Scenari 5 Anni</h3>
                <p style={{ fontSize: 11, color: C.t3, marginBottom: 16 }}>Ottimistico / Base / Pessimistico (k cumulati)</p>
                <ResponsiveContainer width="100%" height={260}>
                  <ComposedChart data={scenari} margin={{ top: 10, right: 10, left: 10, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                    <XAxis dataKey="anno" fontSize={12} stroke={C.t3} />
                    <YAxis tickFormatter={v => v + 'k'} fontSize={11} stroke={C.t3} />
                    <Tooltip formatter={v => [v + 'k']} />
                    <Legend />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
              {[
                { label: 'DSO ASP', valore: '135 gg', target: '< 120 gg', alert: 'GIALLO' },
                { label: 'DPO Fornitori', valore: '95 gg', target: '< 120 gg', alert: 'VERDE' },
                { label: 'DSCR', valore: '1.15x', target: '> 1.2x', alert: 'GIALLO' },
                { label: 'Copertura Cassa', valore: '1.8 mesi', target: '> 2.0 mesi', alert: 'GIALLO' },
              ].map((kpi, i) => {
                const ac = alertCol(kpi.alert);
                return (
                  <div key={i} style={{ ...cardS, textAlign: 'center' }}>
                    ...
                    <div style={{ marginBottom: 6 }}>
                      <span style={{ background: ac.bg, color: ac.color, padding: '2px 10px', borderRadius: 12, fontSize: 11, fontWeight: 600 }}>{kpi.alert}</span>
                    </div>
        ...
        {tab === 'analisi' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={cardS}>
              <h3 style={{ fontSize: 15, fontWeight: 700, color: C.t1, marginBottom: 4 }}>Variance Analysis — Decomposizione Scostamenti</h3>
      ...
      <div style={{ textAlign: 'center', padding: '16px 0', fontSize: 11, color: C.t3, borderTop: '1px solid ' + C.bordo }}>
        Karol CdG v2.0 — Gruppo Karol S.p.A. — Dati simulati test pipeline
      </div>
    </div>
  );
}
